...
#提取响应内容-通过调用 response 响应对象的 read() 方法提取 HTML 信息，该方法返回的结果是字节串类型(bytes)，因此需要使用 decode() 转换为字符串
...
# import osimport timeimport requestsfrom bs4
...
# 将数据项转换为一个元组    row = (school_name, address)
# 将数据行写入 Excel 表格
# ws.append(row)#
# 将文件保存到桌面desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
# file_path = os.path.join(desktop_path, "重庆市小学爬取.xlsx")
# wb.save(file_path)print('数据爬取完成！')
